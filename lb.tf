...
resource "azurerm_lb_rule" "lb_public_http" {
  ...
  probe_id                       = azurerm_lb_probe.lb_public_probe_http[0].id
  frontend_ip_configuration_name = "${var.env_prefix}mainip"
  backend_address_pool_id        = azurerm_lb_backend_address_pool.lb_public_backend[0].id
  protocol                       = "Tcp"
  frontend_port                  = 80
  backend_port                   = 80
}
...
resource "azurerm_lb_rule" "lb_private_http" {
  ...
  loadbalancer_id                = azurerm_lb.lb_private[0].id
  name                           = "${var.env_prefix}loadbalancer-rule-http"
  probe_id                       = azurerm_lb_probe.lb_private_probe_http[0].id
  frontend_ip_configuration_name = "${var.env_prefix}mainip"
  backend_address_pool_id        = azurerm_lb_backend_address_pool.lb_private_backend[0].id
  protocol                       = "Tcp"
  frontend_port                  = 80
  backend_port                   = 80
}
